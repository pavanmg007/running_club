...
const MarathonDetail = () => {
    ...
    const formattedDate = new Date(marathon.date).toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
    ...
    const handleShare = async () => {
        const shareData = {
            title: marathon.name,
            text: `Join ${marathon.name} on ${formattedDate}${marathon.location ? ` at ${marathon.location}` : ''}`,
    ...
    return (
        <>
            <Helmet
                title={marathon.name}
                description={`Join ${marathon.name} on ${formattedDate}${marathon.location ? ` at ${marathon.location}` : ''}. Available categories: ${categories.map(c => c.name).join(', ')}. Register now and be part of this exciting running event!`}
                url={`https://runaranga.club/marathons/${id}`}
            />
